...
Shader "1UP/Magic Outline/NenLit"
{
	Properties
	{
		...
		//_MulColor("Intensity", Range(0.0, 5)) = 2
        ...
        _Brightness("Extra Outline Brightness", Range(0.5, 10)) = 4
		_Edge("Extra Outline Edge", Range(0.0, 1)) = 0.1
		_RimPower("Extra Outline Power", Range(0.01, 10.0)) = 3.0

		_Outline("Outline width", Range(0, 5)) = 0.128
	...
	SubShader
	{
		...
		Pass
		{
			...
			#define _MulColor 2.0

			#pragma shader_feature _LDIR_ON
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
                ...
                //float2 uv = float2(i.pos.x* _Scale - (_Time.x * _SpeedX * 0), i.pos.y * _Scale - (_Time.x * _SpeedY)); 
